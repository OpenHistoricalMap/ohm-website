...
module Accounts
  class AuthDeletionsControllerTest < ActionDispatch::IntegrationTest
    ...
    def test_routes
      assert_routing(
        { :path => "/auth/provider/deletions/confirmation_code", :method => :get },
        { :controller => "accounts/auth_deletions", :action => "show", :provider => "provider", :confirmation_code => "confirmation_code" }
      )
      assert_routing(
        { :path => "/auth/provider/deletions", :method => :post },
    ...
    def test_show_facebook_invalid_code
      get auth_deletion_path(:provider => "facebook", :confirmation_code => "invalid")
    ...
    def test_create_facebook
      ...
      assert_equal auth_deletion_url(:provider => "facebook", :confirmation_code => js["confirmation_code"]), js["url"]
    ...
    def test_create_unsupported
      post auth_deletions_path(:provider => "unsupported")
      assert_response :not_found
    end
  end
end
